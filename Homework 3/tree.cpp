...
int countIsIn(const double a1[], int n1, const double a2[], int n2) {
    if (n1 <= 0) {
        return 1; // a1 is empty
    }
    if (n2 <= 0) {
        return 0; // a2 is empty
    }
...
void order(double a[], int n) {
    ...
    int midIndex = n / 2, firstNotGreater, firstLess;
    divide(a, n, a[midIndex], firstNotGreater, firstLess);
    order(a, firstNotGreater);  // sort up to firstNotGreater
    order(a + firstLess, n - firstLess);    // sort the array after firstNotGreater (i.e. starting at firstLess)
}